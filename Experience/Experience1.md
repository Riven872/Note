...
### 3、优化 ElasticSearch 的搜索性能

1. 集群和硬件优化
    1. 负载均衡：ES 是分布式多节点的架构，因此需要确保查询负载在集群中要均衡分配
    2. 硬件资源：根据需要增加 CPU、内存或改善 I/O 性能（如使用 SSD）
2. 查询优化
    1. 使用过滤器：对于不需要评分的查询条件，使用 filter 而不是 query，因为 filter 可以被缓存以加快后续相同查询的速度
    2. 查询尽可能少的字段：只返回查询中需要的字段，减少数据传输和处理时间
    3. 使用 term 查询文本：term 查询比 match 快，因为 term 查询不需要对查询内容进行分词，term 查询直接查找精确的值，索引定位效率高
    4. 避免使用通配符、正则表达式：以通配符开头的查询会非常消耗性能
