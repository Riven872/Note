...
1. 背景：
    1. 哈希主要用于文件校验、数字签名、密码保护和分布式机器寻址
    2. 其中在分布式机器寻址时，如果一开始设定集群有 128 台机器，此时通过简单的哈希运算就可以映射到对应的节点，但如果集群进行扩容到 256 台机器，再采用同样的哈希函数前必须进行重散列操作，也就是 256 台机器的地址要重新分配，成本巨高
    3. 因此一致性哈希通常用于分布式系统中数据分片和负载均衡的算法，目的是在节点的动态增加或删除后，尽可能减少迁移数据和重新分布的成本
    4. 哈希和一致性哈希的共同点都是取模运算，区别在于哈希是对节点的数量进行取模，所以无法适用于动态场景。一致性哈希对固定值 2^32 进行取模运算
2. 实现一致性哈希
    1. 构造一个虚拟的哈希环结构，然后划分成固定数量的 2^32 个节点，那么节点编号就是 0 ~（2^32 - 1）
    2. 然后将 128 台机器作为节点映射到这些虚拟节点上，如 `hash(server_01)%2^32 = 1003、hash(server_02)%2^32 = 5093、hash(server_03)%2^32 = 9681`。可以通过机器 IP 或主机名进行映射。
    3. 当客户端发起请求时，同样的进行哈希运算，如 `hash(request_01)%2^32 = 3230` ，哈希之后的值必定落在两个节点之间，此时在哈希环上顺时针移动，最先遇到的机器就是该请求映射到的机器，如哈希 3230 后进行移动会首先遇到编号为 5093 的机器
3. 机器扩容
    1. 当加入新节点时，首先通过一致性哈希将该节点加入到哈希环中，如 `hash(server_04)%2^32=2332`，这样就会有一部分的请求再次哈希后，顺时针遇到的第一台机器就变了，所以会寻址到新添加的机器
    2. 相比于普通的哈希算法，在增加服务器后，受影响的范围非常小，其他数据不需要做调整
4. 优点
    1. 均衡：在增加或删除节点时，一致性哈希只会影响少量请求的寻址，保证了负载均衡性
    2. 高扩展：当节点数发生变化时，对于已经在哈希环上的节点不受影响，只有部分请求需要重新寻址，不会影响到所有请求
5. 缺点
    1. 哈希倾斜：在节点数较少的情况下，由于哈希空间有限，可能会导致节点分配不均匀，如两个节点之间哈希了 3000 个请求，另外两个节点之间只哈希了 10 个请求，导致负载不均衡
    2. 节点的频繁变更：如果频繁增加或删除节点会导致频繁的变更寻址，给系统造成压力
6. 哈希倾斜
    1. 解决哈希倾斜最好的方法就是增加节点，增加哈希环上节点的离散性
    2. 如果没有这么多服务节点，可以将一个服务映射成多个虚拟节点，将这些虚拟节点在哈希环上尽可能的散列，当请求映射到虚拟节点时，再找到原始的物理节点处理请求即可
7. 分表中一致性哈希的应用
    1. 首先将表进行一致性哈希，变成哈希环上的节点
    2. 当添加新数据时，通过哈希分片键，得到哈希环的一个节点位置，然后顺时针找到下一个遇到的表节点，该节点就是这条数据存放的位置
8. 2^32 固定值的选取
    1. 2^32 提供了约 43E 个离散的哈希槽，即使节点再多，节点再进行虚拟重新映射也够用，且哈希冲突也低
    2. 目前机器普遍为 64 位、32 位，但如果使用 2^64 会有严重的边际效应，因为 2^32 已经够用，再多的哈希槽只会双倍增加计算和存储成本
    3. 如果使用 2^16 只有 65536 个槽位，节点可能够用，但产生哈希冲突的概率显著上升，会导致数据倾斜严重



### 14、基于本地消息表实现分布式事务

1. 以下单成功后会员积分增加为例（即用户下单后，用户积分也要对应增加，只要其中一个失败，则没满足一致性。且下单与积分是两个系统），且采用定时任务扫描消息表的方式实现。
2. 创建本地消息表，包括发送方、创建时间、消息状态等信息
3. 下单成功后，立马写本地消息表，且保证在一个事务中，这样就保证了下单的同时也能成功记录本地消息。此时需要在事务外发送 MQ 消息。如果消息发送成功则同步修改本地消息表的状态（已发送），如果发送失败也不影响事务的提交（发送失败），但是需要定时任务将发送失败的事件查出来并重新发送。
4. 消息发出去之后等待消费者处理，如果处理成功则修改本地消息表的状态（成功），然后进行业务处理到达终结态或继续向下游发送 MQ 消息（同上需要记录本地消息表）
5. 最初始的操作有事务控制，失败的话会回滚且不会发送 MQ。如果后续的 MQ 消费失败，会有定时任务不断扫描消息表，对于未成功的消息进行重新投递，因此后续的操作非常难实现回滚的操作
6. 本质是上游业务结束后，通过 MQ 通知下游操作，同时需要扫描本地事务表查询是否执行成功。而本地事务表是在执行业务时同步更新，如果投递失败，MQ 的失败投递回调接口会回写状态为发送失败。
7. 优点：
    1. 可靠性高：可以将本地消息和本地业务操作放在一个事务中执行原子性的操作，保证消息的可靠性
    2. 可扩展性高：消息发送和本地业务执行分开处理，进行解耦
    3. 适用范围广：因为业务之间没有耦合性，因此各种业务之间都适用
8. 缺点：
    1. 实现复杂度高：需要设计消息和业务同一事务机制，并且需要相应的异常处理和补偿操作
    2. 系统性能受限：核心点在于需要定时任务扫描本地事务表，且中间会有 MQ 消息的流转，因此性能较低
    3. 回滚困难：该方案适用于上游成功那么下游一定成功的场景，否则会一直重复投递。如果需要整个事务回滚则可能需要更复杂的 TCC 或者 2PC 方案
    4. 参与方多不合适：本地消息表中数据只有一条，如果这条数据既要完成积分的增加，又要完成库存的扣减，这种多参与方的不合适。虽然可以在消息表中增加多个状态字段，表示多个参与者的状态，但耦合度就过高
    5. 性能损耗：消息堆积扫表慢，集中时的定时任务扫表会影响正常业务，且会存在延迟问题
...
1. `query`：定义了要匹配哪些文档以及如何计算它们的相关性得分 (_score)

    1. `must` 子句：必须满足所有子查询（逻辑 AND），所有 `must` 子句的得分都会参与计算，共同影响最终得分。

    2. `filter` 子句：必须满足所有子查询（逻辑 AND） ，子查询以过滤模式运行，不贡献得分。结果会被缓存，性能极佳

    3. `should` 子句：应该满足子查询中的一个或多个（逻辑 OR），通常匹配的 should 子句越多，得分越高。但在 must 存在时，角色会转变

    4. `must_not`子句：必须不满足所有子查询 (逻辑 NOT)，与 filter 类似，不贡献得分，结果会被缓存。

    5. `bool` 查询组合逻辑运算符查询示例
            ...
            GET /products/_search
            {
              "query": {
                "bool": {
                  ...
                  "filter": [ 
                    { "range": { "price": { "gte": 1000 } } }, // 并且价格必须 >=1000
                    { "term": { "status": "上市" } } // 并且状态必须为“上市”
                  ],
                  "should": [
                    { "term": { "has_5g": true } }, // 如果支持5G，会提高得分（排名更靠前）
                    { "term": { "brand.keyword": "华为" } } // 如果是华为品牌，也会提高得分
                  ],
                  "must_not": [
                    { "term": { "category.keyword": "配件" } } // 必须不属于“配件”类
                  ]
                }
              }
            }
    ...
    6. match 和 term 的区别
        ...
        1. match 会进行全文查询，会进行分词，且文档包含分词后的任何一个词项即视为匹配，因此相当于模糊搜索、相关性搜索，一般用于 text 类型的字段，如商品名称或者日志信息查询。需要计算相关性得分，因此性能较低
        2. term 会进行词项查询，不会分词，一般用于精确查询，一般用于 keyword 类型查询，如用户标签、状态、类型、ID、用户名等。不需要计算相关性得分，性能相对较高，ES 的数据缓存后性能更高

2. aggs：用于对查询命中的文档进行统计分析，生成一些汇总信息。它基于 query 的结果集工作

    1. 用的不多，再问的时候补充，一般是把数据查询到内存中处理，在 ES 中处理性能不高

3. sort：定义了搜索结果的排序方式。默认情况下，结果按 _score 降序排列
...
1. 消息体中约定一个业务字段，该字段用来做幂等校验防重
2. 使用消息中间件天然的 msg_id 来做幂等，但是可能存在同样的消息重复发送，msg_id 不同导致出现重复消费的问题，所以最好是在消息体中约定唯一的幂等字段



### 2、重复下单

1. 向外提供一个生成 token 的接口，当用户每一次访问订单页或下单页的时候都去请求接口获取唯一的 token，之后在本页面的其他操作中，都需要携带这个 token。如果页面没有刷新，则 token 不变
2. token 只可使用一次，这个 token 不仅可以在当前页面做防重控制、重复点击、重复下单，还有效的避免恶意盗刷接口
3. Redis 分布式锁 + token 验证应对高并发下的场景
    1. 生成 token 时，采用较小的颗粒度，如当前商品 + 当前用户 + 随机数生成（或订单号 + 当前用户），并放到 Redis 中
    2. 在后续场景中，防止高并发导致 token 被使用多次或同时校验成功，请求时取出 token 作为分布式锁的 key
    3. 上锁的同时查询该 key 是否已经有使用标志，如果有说明已经有业务成功执行，直接返回即可
    4. 如果查询后发现未使用，则进行正常的业务处理，处理完毕后刷新 key 的值，标识为已使用
    5. 这样即可以防止大量 token 生成影响性能，也能应对高并发场景下多线程同时使用 token
4. token 要设置合理的过期时间，防止用户长时间停留页面导致永久 token 带来额外的风险（如恶意脚本利用）



### 3、基于 Redis ZSET 实现滑动窗口限流

1. 滑动窗口优点是可以在规定时间内平滑的控制流量，而不是简单的设置固定的请求数或速率，可以让系统可以更灵活的应对突发流量或峰值流量

2. 基于时间进行统计，在每一次有请求进来的时候，记录请求的时间戳和请求的数据，然后在统计窗口内请求的数量时，只需要统计窗口内的被记录的数据量有多少条

3. 步骤：

    1. 假定 A 接口一分钟（窗口时间）峰值调用 100 次

    2. 然后将 A 接口作为 key 在 Redis 中存储（key 一般为限定的资源名，接口限流一般采用接口名作为 key）

    3. 分数 score 设置为当前请求的请求时间

    4. member 采用请求的数据进行 MD5 或其他唯一值存储，防止并发时因为 member 相同导致无法添加重复值的情况

    5. 这样每有一个请求进来，就会定义出一个新的 ZSET 然后存储到 Redis 中

    6. 限流的主要思想是只保留在指定时间窗口内的请求记录，而丢弃窗口之外的记录，这样就可以通过 ZSET 的排序命令删除分数值小于窗口起始时间戳（当前时间戳 - 60s）的数据

    7. 最后使用 ZCARD 命令获取当前 key 的 ZSET 集合中的成员数量，那么这个数量就是这个窗口时间内的请求量

    8. 即当有请求进来时，判断窗口时间内请求量是否大于峰值，大于则拒绝响应，小于则可以将当前请求加入进来

    9. 同样因为以上可能会有高并发的场景，因此可以采用 Lua 脚本来一次性执行原子命令
...
### 4、高并发场景下库存扣减的超卖与少卖问题

1. 两个并发线程同时查询库存时都获取的结果为 1，经过库存校验后分别进行库存扣减，最终就会导致库存被扣成负数。因此在并发场景下需要保证查询与扣减的原子性与有序性

2. 数据库方案：

    1. 如果采用悲观锁会导致大量的请求阻塞且排队，会给数据库造成很大的压力。如果采用乐观锁，可以通过版本号来控制有序执行，但因为版本号是表字段，实际 update 操作时会加行锁，也会对数据库性能有影响

    2. 因为 SQL 语句的执行是原子化的，因此可以通过数据库本身的 SQL 语句进行控制，但仍然依赖数据库，高并发执行 update 操作依然会发生阻塞

    3. ```sql
        UPDATE table_foo
        SET quantity = quantity - #{count}
        WHERE sku_id = '123' AND quantity >= #{count} -- 幂等校验
        ```

3. Redis 方案：

    1. Redis 是单线程执行命令，且 Lua 脚本的执行过程是原子性的，因此可以同时保证原子性和有序性

    2. 当上架新商品或增加商品数量时，同步将数据库内的库存值添加到 Redis 中

    3. 查询库存时，通过 Redis 查询出当前剩余的库存量，并顺序判断是否足够扣减，足够则进行扣减并更新 Redis 内的库存数量，否则返回库存不足

    4. Redis 性能相对较高，采用 Lua 脚本的方式也可以在高并发环境下正常运行
...
4. 一致性保证：
    ...
    1. 在实际生产环境中，数据库和 Redis 方案通常会结合使用，目的是使用 Redis 抗高并发流量进行正确的扣减，然后再将库存数据同步到数据库中，将库存数量持久化，防止 Redis 挂了导致数据丢失
    2. 一般是先在 Redis 中做扣减，然后发送一个 MQ 消息，消费者收到消息后做数据库中库存的真正扣减及后续的业务逻辑操作
    3. MySQL 中的扣减语句作为兜底行为，MQ 的消息会通知 MySQL 需要扣减的库存量，此时 MySQL 进行扣减达成最终一致性，而不是通知扣减后的库存量做同步。所以 MQ 传递的是"要扣减的库存数量"（操作指令），而不是"已扣减完成的库存数量"（结果状态）

5. 职责分离：

    1. Redis 负责实时并发扣减，MySQL 负责数据持久化并通过 SQL 语句进行二次校验，MQ 则负责传递指令进行职责解耦与流量削峰。这样即使 MySQL 二次校验没有通过，Redis 也可以做回滚操作。如果消息是通知已扣减的结果，则无法校验也无法回滚

    2. MQ 消息体通知 MySQL 某商品因为某订单扣减多少的数量，然后 MySQL 进行最终持久化

    3. ```json
        {
          "order_id": "20230703123456",
          "sku_id": 1001,
          "count": 2
        }
        ```
...
6. 少卖问题：
    ...
    1. Redis 实时扣减成功后，会通过 MQ 来达成最终一致性，如果消息丢失或消费失败，就会导致 Redis 中的实际库存扣减了但是 MySQL 中没有扣减，数据不一致产生的结果就是 Redis 发生了多扣，业务问题就是少卖的情况
    2. 消费失败时需要实时通知 Redis 进行回滚操作，并通过定时任务建立核对机制，定时拉取一段时间内的订单记录并将数量和数据库内的库存做对比，差距过大时需要补库存，必要时需要在 Redis 内同步存储流水记录

7. 不使用分布式锁的原因：

    1. 该场景下的主要问题就是原子性与有序性，分布式锁同样可以在锁范围内，进行查库存与扣库存原子操作
    2. 但使用分布式锁会导致每次扣库存时需要多次的与 Redis 通信（尝试获取锁，加锁、读取库存、扣减库存、释放锁等），这样不仅增加了网络延时还提高了系统的复杂性
    3. Lua 脚本本身就是原子、有序性的执行，相当于更高效的分布式锁，一次脚本执行就可以减少多次网络传输和通信的开销
    4. 因此可以使用分布式锁，但 Lua 脚本能完成同样的功能而且开销更小



### 5、MySQL 热点数据高效更新

1. 要求在 MySQL 数据库层面抗住高并发更新数据，本质是高并发进行热点行的更新，主要目的是避免 Redis 数据不一致、少卖等问题，以库存更新为例

2. 库存拆分

    1. 把一个大的库存拆分成多个小库存，拆分后，一次库存扣减动作就可以分散到不同的库、表中进行，降低锁颗粒度提升并发
    2. 实现比较简单，但是存在碎片问题、库存调控不方便

3. 请求合并（缓冲记账）

    1. 把多个库存扣减请求合并成一个，进行批量更新而不是单条单条更新
    2. 实现简单，但是只适用于异步场景或者分析后可以合并的场景

4. update 转成 insert

    1. 直接插入一次占用库存的记录，然后异步统计剩余库存，或者通过 SQL 统计流水的方式计算剩余的库存
    2. 优点是没有 update 操作，没有锁冲突，但是 insert 时不好控制超卖、insert 后不好统计剩余库存

5. **改造 MySQL**

    1. 大幅度优化对于热点行数据的 update 操作性能，大量并发的 update 排队执行，以减少大量行锁造成的并发性能下降。在执行层让 update 排队要好于数据库层面排队，不需要自旋也不需要抢锁

    2. Inventory Hint（RDS 阿里云数据库热点行并发更新的原理）

        1. update 语句增加特殊的 hint 语句
            ...
            -- 以下三个 Hint 只有阿里云 RDS 支持
            -- COMMIT_ON_SUCCESS 当前语句执行成功就提交事务上下文
            -- ROLLBACK_ON_FAIL 当前语句执行失败就回滚事务上下文
            -- TARGET_AFFECT_ROW(NUMBER) 如果当前语句影响行数是指定的就成功，否则语句失败
            
            -- hint 是 MySQL 的特殊语法，影响数据库引擎选择何种索引、是否使用缓存等，目的是优化性能
            ```

        2. 使用 `COMMIT_ON_SUCCESS` 标记一条 SQL，告诉 MySQL 内核该行可能是热点更新数据，那么一定时间间隔内，MySQL 内核层就会自动识别带此标记的更新操作，并将收集到的更新操作按照主键或者唯一键进行分组，这样更新相同行的操作就会被分到同一组中

            1. 为了进一步提升性能，在实现上会使用两个执行单元。当第一个执行单元收集完毕准备提交时，第二个执行单元立即开始收集更新操作。当第二个执行单元收集完毕准备提交时，第一个执行单元已经提交完毕并开始收集新一批的更新操作，两个单元不断切换，并行执行

        3. 根据热点进行分组后，会有 3 个关键优化点

            1. 减少行级锁的申请等待
                1. 在同一组中，需要更新的都是同一条记录，那么根据 SQL 的提交顺序就可以排队，然后第一条 SQL 语句执行前，尝试获取目标行的锁，如果获取成功则开始执行，这一组后续的更新操作也会尝试获取锁，但先会判断是不是第一条更新操作获取到了，如果是就不需要等待直接获取锁，这样就可以大大降低申请行级锁的阻塞时长（有种 synchronized 偏向锁的味道。。。。）
            2. 减少 B+ 树的索引遍历操作
                1. MySQL 每次执行查询时，需要遍历索引树才能定位到目标数据行，数据表越大，遍历时间越长。如果针对热点行更新操作做了分组之后，只需在每组的第一条 SQL 执行过程中，通过遍历索引定位数据行，之后把这些数据行缓存到 Row Cache 中并进行修改，在同组的后续操作中，也不需要再索引查询了，直接从 Row Cache 中获取数据并修改，这样就大大降低了遍历索引树的耗时
            3. 减少事务提交次数
                1. 如果高并发更新单行数据，多条 update 语句会有多个事务，那么每一个事务都要单独做一次提交。有了分组、排队、组提交之后，就只需一组中的并发操作都执行完，然后做一次组提交即可，大大降低了事务的提交次数
...
1. tryLock
    1. 尝试获取锁，如果获取成功则直接返回 true，如果无法立即获取到锁，会按照给定的 `waitTime` 参数进行阻塞尝试获取，如果还是获取失败，则返回 false
    2. 如果没有指定 `waitTime`，则直接返回 false
    3. 相当于非阻塞式的，立即回应获取锁是否成功失败，指定 `waitTime` 会进行阻塞并尝试继续获取锁
2. lock
    1. 以阻塞的形式获取锁，如果获取不到，那就一直阻塞直到获取锁成功
    2. 底层采用的是 Redis 发布订阅机制，不会忙自旋。如果获取失败，那么线程就会进入阻塞休眠状态，并开启订阅，直到收到解锁通知并被唤醒后，才通过 Lua 脚本尝试去获取锁，如果再次获取失败，那么线程会再次挂起并订阅等待唤醒
3. 二者的差异不大，主要是 tryLock 指定了时间后，会一直检查等待时间 `waitTime` 是否耗尽，然后才是阻塞 - 订阅 - 尝试获取锁这套流程，相比 lock 而言，不会无限期的阻塞等待下去，会有兜底的时间限制。而且 tryLock 会主动的尝试获取锁，而不是被动的等待唤醒，因此可以说是两套方案（订阅被动唤醒和主动尝试获取锁）
...
1. ChatModel / ChatClinet
    1. 与 LLM 大模型进行交互通信的统一入口
    2. 抽象层级：定义了一个与 LLM 供应商无关的 API 接口（`call()、stream()`），这就意味着无论是对接 OpenAI 还是 DeepSeek，业务代码只与 ChatModel交互。
    3. 多样性：为每个支持的模型提供了具体的实现，如对接 OpenAI 有 OpenAiChatClient、对接阿里百炼有 DashScopeChatClient。这些实现类负责处理各自模型特定的 API 格式、认证方式和响应解析。
    4. 高级用法：ChatClient 是在 ChatModel 基础上进一步封装了更复杂的交互模式，在处理最基础的请求响应基础上，内置了函数调用、结构化输出的支持、RAG 知识库搜索等高级功能
2. PromptTemplate
    1. 动态生成结构化的提示词
    2. 提示词模板拼接：作用是将提示词的构建从原始基础的字符串拼接转换为模板渲染的层面，也就是占位符和 SpEL 表达式的支持。
    3. 标准化与复用：通过模板，可以自行定义标准化的提示词模板，只需要根据业务场景传入不同的占位符参数即可，因此模板可以在整个应用中复用
    4. 上下文感知：在链式调用中，PromptTemplate 能够从上游组件的输出中获取上下文，并动态的填充到当前的模板中。
3. Structured Output Converter
    1. 将模型返回的非结构化文本自动映射到 Java 对象中
    2. 类型安全：模型返回的数据可以直接反序列化成对象，而无需手动解析 Json 或文本，保证实体对象内成员的安全
    3. 底层原理：实际是在提示词中附加指令，要求模型以特定的格式返回（如 Json），然后框架利用如 Jackson 库等将返回的 Json 字符串反序列化成目标实体
4. VectorStore
    1. 抽象了一套统一的向量 API 来操作各种向量数据库
    2. 统一性：无论是 Redis、PgVector 还是 Es，都可以用相同的 VectorStore 接口进行 add、search 操作
    3. 与 `EmbeddingClient` 协作：VectorStore 本身不负责文本到向量的转换，因此交由 EmbeddingClient 来完成转换操作
    4. 向量检索：`similaritySearch` 用于根据查询向量数据进行相似度检索，是实现 RAG 的基础
5. Advisors
    1. 在 AI 交互的请求和响应流中提供面向 AOP 切面的干预能力
    2. 拦截器模式：本质是在与 AI 交互过程中的拦截器，可以在请求被发送到模型之前修改或增强 Prompt 或在响应返回给调用者之后调用 MCP 进行自定义的业务逻辑处理
    3. 通用的横切面：非常适合处理与核心业务无关但普遍存在的功能（如系统提示词的注入、对话历史的管理、输出内容的合规性过滤等）



### 2、Spring AI 如何简化与 LLM 集成的流程

1. 统一的抽象层与统一配置
    1. 通过 SpringBoot 的自动配置，将 ChatModel 配置类进行自动装配，并读取 yml 配置文件初始化 ChatModel 所需要的配置（如 apiKey、LLM name 等）
    2. 也可以手写配置类与 Bean，通过代码的形式完成 Bean 的参数初始化
    3. 整个 IOC 容器加载完毕后，可以通过 `@Resource` 在任何地方注入 ChatClient 并与 LLM 进行交互，而无需关心具体的实现
2. 与 Spring 生态无缝集成
    1. 所有 AI 组件（`ChatClient`, `VectorStore`, `EmbeddingClient`）都是 Spring Bean，可以轻松地被其他 Bean 注入，也可以注入依赖（如 `DataSource`）



### 3、ChatModel 与 ChatClient

1. ChatModel
    1. 是模型层的抽象，代表 LLM 本身的能力，是 Spring AI 框架定义的最核心的一个接口，用于标准化与不同 LLM 供应商的交互，关注点是：向 LLM 发送一个请求，并得到一个回应
    2. 适用场景：
        1. 需要系统提示词：需要为 AI 固定一个角色或行为模式
        2. 需要多轮对话上下文：对话记忆以实现连贯的交流
        3. 需要精确的请求结构：需要构建复杂的、包含多种类型信息的 Prompt（系统、用户、助手）
        4. 需要监控和可预测性：需要记录和分析每次调用 Token 使用量、LLM 版本、响应时间等元数据
        5. 需要处理候选回复：LLM 可能会有多个候选答案，需要从中进行选择或处理
2. ChatClient
    1. 客户端与应用层的抽象，是在 ChatModel 的基础上构建的更高级封装更多功能的组件，因此提供了更多便捷功能与流式支持，关注点是：执行一个对话任务
    2. 适用场景：
        1. 简单的回答功能：单次、无上下文的回答，不需要复杂的角色设定
        2. 实现流式响应：实时将 AI 的回复呈现给用户
        3. 简化 API：不关心复杂的 Prompt 和 ChatResponse 对象，只关心最终的文本内容
        4. 高级功能：可以快速开发复杂功能的场景，如智能客服、连接外部工具等
3. 通常二者会结合使用，通过 ChatModel 偏底层的控制来完成所需功能的开发，最后在 Controller 层



### 4、SSE

1. SSE（Server-Sent Events） 是一种允许服务器向浏览器或其他客户端推送实时更新的技术，是一种单项通信机制，服务器可以主动向客户端发送数据，而客户端无需频繁轮询服务器请求数据
    1. 单向通信：仅支持服务器到客户端，客户端不能向服务器发送消息。如果需要双向通信可以结合 WebSocket
    2. 基于 HTTP：SSE 不需要额外的协议支持，兼容性好
    3. 自动重连：客户端在连接中断后会自动尝试重新连接
    4. 数据格式：SSE 数据以特定的格式发送，每条消息以 `data:` 开头
    5. 事件类型：可以为每条消息指定事件类型，客户端可以通过监听特定事件类型来处理不同的消息
    6. Spring MVC（Spring Web）底层是基于 Servlet 实现的，它是使用 SseEmitter 技术实现 SSE 协议实现流式输出的
2. 但 Spring AI 的流式输出是 Spring WebFlux 基于 Reactive Streams 规范实现的，后续再补充 Spring WebFlux
...
1. 系统角色：设定 AI 的行为边界，指导 AI 的行为和响应方式，设置 AI 如何解释和回复输入的参数或规则
2. 用户角色：接收用户原始输入，代表用户输入向 AI 提出的问题、命令或陈述
3. 助手角色：AI 返回的响应信息，用它可以保证确保上下文能够连贯的交互
4. 工具/功能角色：连接外部服务，可以进行函数调用或 MCP 调用，如支付、查询数据等操作



### 2、提示词的安全性

1. 本质是用户输入的提示词中包含了模型能够识别的指令符号（如模仿系统提示词级别的格式等），会导致模型错误的将用户数据解析为指令，从而偏离了预设的执行路径
2. 构建防御体系：
    1. 输入清洗与转义：在将用户输入嵌入到预设提示词模板之前，对所有的用户输入进行扫描，将其中的潜在指令符号（如换行符、引号以及可能被 LLM 理解为指令的特定词语和符号）进行转义和替换，这样 LLM 在读取时，这些符号就失去了原先的指令功能变成了纯文本内容（如添加转义符、替换成特定的标记等）
    2. 角色与指令隔离：在构建 Prompt 时严格区分系统指令区和用户输入区。系统指令区使用 `SystemMessage` 以最高权限定义 AI 的角色和行为准则。用户输入则被严格放置在 `UserMessage` 中。通过这种结构上的隔离，LLM 也能根据角色区分从而降低将其误执行为系统指令的概率
    3. 输出过滤与校验：LLM 返回的结果不可直接信任，需要设立一个后处理环节，检查输出中是否包含敏感信息，如返回了异常信息，可以判定本次交互可能有安全隐患从而触发重试或向用户返回错误信息



### 3、提示词的模板管理

1. 外部化与配置化：将提示词模板从代码中剥离出来，存储在独立的文件中（如 YML 或数据库），这样修改提示词就无需重新编译，并建立一个提示词模板的配置中心，业务代码通过每个模板的 ID 来请求和渲染模板
2. 版本控制与灰度发布：用 Git 管理模板文件，便于追踪变更和回滚每一个提示词模板。并可以通过配置中心实现模板的灰度发布，小比例流量使用新模板，观察效果和指标，无误后切量发布，能极大降低线上风险
3. 参数化与变量渲染：设计模板引擎工具类，模板中使用占位符。在运行时，应用可以实时的将用户对话产生的问题、检索到的知识、当前日期等传入引擎工具类中，由引擎进行动态的模板渲染，生成最终的 Prompt，保证了提示词的动态性和灵活性



### 4、领域知识融入

1. 目的是为了让 LLM 在特定领域有更好的表现，需要将领域知识融入提示词中
2. 通过检索增强生成（RAG）进行处理，先搜索再回答
    1. 知识库：将企业内部的领域知识（文档、手册、数据库等）进行切片、向量化，并存入专门的向量数据库
    2. 实时检索：当用户提出问题时，系统首先将用户问题也转化为向量，然后在向量数据库中进行相似度搜索，找到于问题最相关的几个知识片段
    3. 上下文增强：将这些检索到的知识片段和用户原始的问题一起构成一个新的、信息更加丰富的 Prompt，发送给 LLM
    4. 指令化生成：在 Prompt 中明确指令模型“基于以下参考资料来回答问题”，并要求注明答案出处。这样 LLM 不再依赖于固有的、可能过时或不准确的知识，而是基于提供的领域知识来生成答案。也解决了模型的“幻觉问题”和知识陈旧的问题
3. 通过微调（Fine-tuning）进行重定位
    1. 成本太高了。。。。。。估计很少有企业会这么做，问的时候再补充
...
1. 对话状态跟踪：
    1. 维护一个与当前对话会话（Session）绑定数据结构。这个结构不仅存储原始的对话记录，还要提炼出当前的对话状态（当前讨论的核心关键字、用户已明确表的意图、已经收集到的关键信息片段以及未明确的信息）
2. 上下文窗口与摘要压缩：LLM 上下文长度有限，不能无限制的将所有历史对话对放进 Prompt
    1. 关键信息提取：从历史对话中，只提取对理解当前问题最关键的信息，如实体名、数字、决策结论等，而不是完整的对话原文
    2. 主动摘要：在对话达到一定轮数或检测到话题转移时，主动调用模型对之前对话历史生成一个简单的、结构化的摘要。在后续的 Prompt 中不再附上全部历史，而是附上这个摘要和最近几轮的对话。
    3. 分层记忆：对会话记忆分为短期记忆（最近几轮的完整对话）和长期记忆（整个会话的摘要或关键事实），每次构建 Prompt 时，将长期记忆和短期记忆组合起来，作为上下文
...
1. 提示词压缩
    1. 无关信息剔除：识别并移除与当前用户查询无关的历史对话或知识片段。这可以通过相似度计算来实现（计算历史对话中每一句与当前问题的向量相似度，保留得分最高的）
    2. 抽象与重述：使用一个更小、更快或专门处理文本的服务，对长文本进行摘要，用更精炼的语言表达相同的意思，压缩成一句包含核心诉求的话，然后再将这段话放入 LLM 的 Prompt 中
    3. 指令精炼：审查系统指令，去除冗余的、重复的或不必要的描述，使用最简洁、最明确的指令语言。需要多次的测试
2. 缓存机制
    1. 精确匹配缓存：以完整的 Prompt 字符串作为 Key，将模型的响应结果作为 Value 缓存起来。适用于输入高度标准化、重复率高的场景（如固定的问答对、模板化任务）
    2. 语义缓存：字面上不完全相同但语义相似的查询，应该得到相同的或高度相似的答案
        1. 当一个新用户查询时，首先将其转换为一个向量表示
        2. 在向量缓存中，搜索是否存在语义相似的缓存项（即计算新查询向量与所有缓存键向量的相似度）
        3. 如果命中则直接返回缓存答案
        4. 如果没有命中则调用 LLM 并将本次查询和结果作为新的缓存想存入向量缓存。这种方式会极大的提高缓存命中率
...
1. 根据会话的生命周期、规模和并发需求，选择不同粒度与性能存储策略。解决用户对话数据的存放问题，内存会不会成为瓶颈
2. 会话级内存存储：
    1. 适用于原型开发、测试环境和用户量极小的短期对话场景
    2. 在 Java 应用内存中，维护一个全局的 `ConcurrentHashMap`，其中 key 为 `sessionID`，value 为该会话的 `ChatMemory` 对象。`ChatMemory` 对象内部使用 `List<Message>` 来存储对话消息
    3. 因为基于内存，应用重启后所有对话状态会丢失，且单机内存容量有限。在集群环境下用户的请求可能会被路由到另一台没有其对话的机器，导致上下文丢失
3. 分布式缓存存储：
    1. 适用于大部分场景，要求会话可恢复、应用可水平扩展
    2. 使用 Redis 作为缓存中间件，将 `ChatMemory` 序列化之后存储到 Redis 中，其中会话 ID 作为 Key。
    3. 用户发起请求时，携带 `Session ID`，后端根据 ID 从 Redis 中读取序列化后的数据，然后反序列化为 `ChatMemory` Java 实体对象，进行本轮对话的逻辑处理，处理完成后，将更新后的 `ChatMemory` 对象重新序列化写回 Redis
4. 数据库持久化存储：
    1. 适用于长期存档、审计、数据分析或用户需要查询数月甚至数年前历史对话记录的强需求场景
    2. 在对话最终结束后（如用户关闭会话窗口），将整个 `ChatMemory` 的内容（消息、元数据等）序列化后存入数据库的表中。同时为了性能，活跃会话依然使用 Redis 进行管理，数据库仅作为冷备处理。



### 2、对话内存溢出

1. 通过主动的内存使用监控和自动化的清理回收机制，防止因对话数据无限增长而导致的 JVM 崩溃或 Redis 被写满导致的服务不可用
2. 对话内存容量限制：
    1. 在每个 `ChatMemory` 对象内，设定一个硬性的上限（最大消息条数或最大字符总数），在达到上限时，采用 FIFO 策略自动移除最旧的几条消息，为新消息腾出空间。保证单个会话的内存占用是可控的。
3. 会话生命周期管理
    1. 为每个存储在 Redis 中的会话数据设置 TTL，每次有读取操作时，刷新这个 TTL，如果一个会话在一定时间内没有任何活动，Redis 就自动将其删除，这样就有效清理了“僵尸会话”
4. 主动过期与清理策略
    1. 应用层设计一个定时任务，定期扫描 Redis 中所有会话 Key。可以根据更复杂的规则进行处理（如即使会话活跃，但如果创建时间超过了一定时间，也强制将其归档并从 Redis 中删除，以释放空间给刚创建的、更活跃的对话）
5. JVM 堆内存监控与告警
    1. 即使主要数据在 Redis，应用本地在反序列化后和处理过程中仍会占用 JVM 内存。需要集成监控系统，对 JVM 的堆内存使用率、GC 频率进行实时监控，设置阈值告警。



### 3、对话状态保持

1. 核心思路：将短暂的对话消息与持久的、结构化的业务状态分离，并通过状态机来驱动复杂的多轮对话流程
2. 实际问题：如在订票机器人中对话说，用户先说“我想订票”，然后系统询问“去哪”，用户回答“北京”，系统再问“什么时候”。如何让系统“记住”正在执行“订票”这个任务，并且已经收集了“目的地”这一关键信息，还缺“时间”
3. 状态与消息分离，将 ChatMemory 分为两部分
    1. 消息历史：存储原始的、逐条的对话记录，用于提供 LLM 生成回复所需的上下文
    2. 对话状态：一个独立的结构化对象 `ConversationState`，用于记录当前对话的业务进度，通常包含各种关键信息的维度，如：
        1. `currentIntent:"book_flight"` （当前意图：订机票）
        2. `collectedSlots:{"destination":"北京"}`（已收集到的信息槽位）
        3. `pendingSlots:["departure_time"]`（待收集的信息槽位）
        4. `currentStep:"asking_departure_time"`（当前步骤）
4. 状态机驱动：
    1. 整个多轮对话由一个状态机来驱动。每个状态对应一个系统需要执行的动作（如询问目的地、确认订单）、状态的转换由用户的输入来触发
    2. `ConversationState` 对象实质上是这个状态机的快照，每次对话交互后，不仅更新消息历史，更重要的是更新并持久化这个状态对象。即使会话中断，恢复时也能立刻知道对话进行到哪一步，该做什么
5. 状态持久化
    1. `ConversationState`对象必须和 `ChatMemory` 一起被序列化并存储到 Redis 中，它是恢复对话流程的关键



### 4、隐私合规问题

1. 在数据生命周期的各个环节（输入、存储、处理、输出）实施最小化原则、加密脱敏和访问控制，以满足个人信息保护法等法规的要求
2. 实际问题：对话中可能包含用户的姓名、身份证号等敏感信息，需要确保这些信息不被未授权访问。
    1. 敏感信息脱密存储：在将对话消息持久化到 Redis 或数据库之前，通过一个过滤器对消息文本进行处理。可以使用正则或 NLP 技术识别出敏感信息，并将其替换为脱敏形式。但是脱敏可能会导致 LLM 理解上下文的能力，因此需要评估选择加密还是脱敏
    2. 端到端加密：对于极高敏感度的业务，可以在客户端就对用户输入进行加密，服务端存储和处理的始终是密文。只有在必须调用某个需要明文的后端服务时才在安全环境中解密。能大大降低数据在服务端泄露的风险。
    3. 数据访问权限与控制：建立严格的权限体系。客服人员只能看到自己负责的会话记录；管理员访问日志需要多重审批且有操作记录。在系统层面需要对数据库和 Redis 的访问进行网络隔离和认证授权。



### 5、ChatMemory 的分层存储

1. 所有用户的全部对话历史都放在 Redis 中会导致成本上升，且大部分是很少被访问的陈旧对话。
2. 热 / 温 / 冷数据分层：
    1. 热数据：当前活跃的会话。存储在 Redis 中，保证最快速度的读写延迟。这也是用户正在进行的对话。
    2. 温数据：最近结束的会话（如 24 小时内）。可以将其从 Redis 迁移到性能型云数据库（如阿里云 RDS），延迟会比热数据稍高，但存储成本低，保证了可以快速访问的同时存储大量的温数据。用于用户刚刚结束对话后查询历史。
    3. 冷数据：历史会话（如 24 小时之后）。将其从数据库迁移到对象存储（如阿里云 OSS）或归档数据库。会存在比较大的访问延迟，但存储成本极低。用于审计、数据分析或用户手动查询很久以前的历史记录。
3. 自动数据迁移：
    1. 降级：每日执行的定时任务，将结束时间超过 24 小时的数据从 Redis 批量导出到数据库，并从 Redis 中删除。另一个定时任务每月执行一次，将超过一年的数据从数据库归档到对象存储。
    2. 升级：当用户请求查看一个已归档的历史会话时，系统先从 Redis 中查询，如果没有命中则触发一个异步任务，从对象存储加载数据到数据库或 Redis 中，并通知用户“数据加载中，请稍等”



### 6、ChatMemory 的消息摘要优化

1. 在长达 100 轮对话中，如果每次都将全部的历史记录作为上下文传给 LLM，会消费巨额的 token，且响应缓慢，可以进行消息摘要的优化。
2. 固定窗口与滑动窗口：
    1. 只保留最近 N 条（如 10 条）的消息作为上下文，可以解决长度问题，但会导致长期记忆丢失
3. 增量式摘要：
    1. 触发时机：当对话轮数达到一个阈值时（如 20 轮），或检测到话题发生明显切换时，触发摘要机制
    2. 摘要生成：将从上次摘要之后的所有新消息发送给一个 LLM 或指定一个算法，基于旧摘要和新消息重新生成一个简洁且包含关键信息的段落。因此核心问题是摘要怎么生成，按照什么规则生成。
    3. 存储与替换：将新生成的摘要文本作为一条特殊的系统消息存放在 `ChatMemory` 中
    4. 上下文构建：在后续与 LLM 交互中，Prompt 的构成变为：系统指令 + 历史摘要 + 最近 k 条原始消息。这样 LLM 既能够通过摘要把握全局背景，又可以通过最近消息理解即时的细节，同时上下文长度被严格控制
4. 结构化摘要：
    1. 可以将摘要转换为结构化的 Json。该实体包含多个关键信息字段，可以让程序提取更加方便精确，也利于后续处理
...
1. 开源和嵌入模型众多，目的是选出更适合自己企业业务的模型（如医疗报告方向、法律条文方向、电商评论方向），错误的模型会导致后续检索效果差
2. 领域适配性评估：
    1. 构建领域测试集：手动整理一个具有本业务“标准答案”的集合。包含一组查询问题以及每个问题对应的一系列相关文档和不相关文档
    2. 计算评估指标：使用不同模型将查询和文档向量化，然后计算与检索精度相关的关键指标
        1. 命中率：在前 K 个结果中找到相关文档的概率
        2. MRR：第一个相关文档出现位置的倒数平均值，衡量系统把正确答案排在前面的能力
        3. NDCG：考虑排序顺序的加权评分，越靠前的相关文档权重越高
    3. 胜出者选择：在相同的数据集和配置下，哪个模型的评估指标最高，哪个就是最适合当前领域的模型
3. 任务目标对齐：
    1. 任务分类：不同的 RAG 任务可能需要不同特性的模型。如检索任务需要模型对`查询-文档`的匹配有高区分度，而聚类任务需要模型对`文档-文档`的相似性要有高区分度
    2. 实践方法：如果主要任务是问答型的检索，就应优先选择在检索任务评测上表现好的模型



### 2、索引构建与更新

1. 如果知识库有千万级文档时，需要快速找到与用户问题相似的几个文档。且对知识库进行新增、修改或删除文档时，需要在更新索引的同时不引起服务的中断。因此要构建一个能够支持海量向量快速相似性搜索的数据结构，同时可以应对知识库的动态变化
2. （查询）近似最近邻索引：
    1. 在海量高维度向量中进行精确最近邻搜索计算量巨大，无法满足毫秒级的响应。因此可以考虑牺牲少量精度来换取巨大的性能提升，算法包括：
        1. HNSW：基于图，可以从粗到精的进行搜索，性能和精度都很好
        2. IVF：先对向量空间进行聚类，形成倒排列表。搜索时只在最相关的几个聚类中心里找，可以大大减少搜索范围
        3. PQ：通过量化技术压缩向量，减少存储和计算开销，常与 IVF 结合使用
    2. 选型权衡：HNSW 查询快但是内存占用高，适合千万级以下的数据；IVF + PQ 内存占用小，适合十亿级数据，但需要训练且精度相对较低
3. （更新）增量更新：
    1. 实时写入：对于新增的文档向量，直接插入到索引中。部分索引库支持这种能力，但频繁的实时插入可能会轻微影响索引结构，长期会导致性能下降
    2. 微批处理：设定一个时间窗口，将这段时间内新增或变更的文档批量写入索引
    3. 双索引+切换：构建一个背景索引进程，持续从主数据源同步数据并构建新索引。构建完成后，在某个低峰期，将线上查询流量从旧索引原子性切换到新索引。旧索引保留一段时间用于回归。可以保证线上服务不受索引重建影响
4. （删除）删除与更新处理：
    1. 采用标记删除策略，为每个向量文档附加一个`is_deleted`的元数据。在查询时，先通过向量索引找到候选集，然后在应用层根据`is_deleted`字段进行过滤。定期在索引重建时，将这些已标记删除的数据彻底清除



### 3、检索精度

1. 单一向量相似度搜索可能不够精准，需要通过多路找回、重排序和查询理解等提升最终效果，真正让系统理解用户的意图，找到最相关的答案
2. 多路找回与融合
    1. 通过的检索方法各有自己的特性，可以进行互补
        1. 路1：稠密向量检索：基于语义相似度，召回语义相关但关键词可能不匹配的文档
        2. 路2：稀疏向量检索：基于关键词匹配，召回关键词匹配度高但语义可能不完全一致的文档
        3. 路3：元数据过滤：根据文档类型、发布时间、作者等条件进行筛选
        4. 融合方法：将多路召回的结果合并、去重，然后进行统一排序