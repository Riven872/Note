...
>
> **2、深度优先遍历：先序、中序、后续（结合栈理解），递归法、迭代法。**
>
> **3、递归遍历三要素：确定递归函数需要的参数和返回值、确定终止条件、确定单层递归的逻辑（因为递归是做同样的操作，只需要单层逻辑就是全部的逻辑）。**
>
> **4、层序遍历：核心是使用队列控制，然后根据队列的每次不同的 size 控制访问每层结点的个数。进而延伸出对其中某一层的全部结点进行处理。**
>
> **5、迭代法前中后序遍历：核心是使用栈控制，因为会先将父结点压入栈中，直到该父结点的所有子树全部遍历完成时，才会访问到父结点。**
...
> **1、注意题目中的对称，是镜像对称还是指完全相同，二者的区别在于对比时两个结点不同。**
>
> **2、迭代法：两棵树同时遍历，将遍历的结点放入栈中，通过栈的特性进行两两匹配。**
>
> **3、递归法：每次递归调用时，放入二者的左子树或右子树或其他规定的参数。**



### [E101. 对称二叉树](https://leetcode.cn/problems/symmetric-tree/)
...
### [M222. 完全二叉树的节点个数](https://leetcode.cn/problems/count-complete-tree-nodes/)
...
1. 与上题相同的思路，保留上一个结点，以便进行数值比对。
1. 当 pre 与 cur 结点数值不同时，直接将计数器 count 重置。如果相同则进行累加，如果超过最大值，则将集合进行清理，并放入该值，如果与最大值想等（众数不只一个），只放入该值，不必清空集合。
4. 注意：
