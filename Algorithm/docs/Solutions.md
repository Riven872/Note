...
> **通常建立一个哨兵节点，放置在首节点之前方便遍历。然后再建立一个 pre 节点，用来保留前驱地址。**
>
> **思想类似于双指针法，遍历指针进行遍历时，若不移除遍历指针节点，那么 pre 节点后移指向遍历节点，然后遍历节点才后移，指向下一个节点。如果移除遍历指针节点，遍历节点后移一位，pre 节点指向新的遍历节点即可。**
...
1. 空间复杂度 1，时间复杂度 n。
2. 只有当遍历节点为 null 时，才会停止遍历，而不是遍历节点的 next 为 null 时停止。
