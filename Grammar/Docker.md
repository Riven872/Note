...
- 容器卷加入参数 `--privileged=true`
- 目的是使container内的root拥有真正的root权限，否则，container内的root只是外部的一个普通用户权限
...
- 与导出容器的区别
    - 导出容器：只保留导出那一刻的状态
    - 数据卷：只要容器内发生变化，则变化同步到数据卷中，不需要手动控制

###### 4、宿主机和容器之间映射添加容器卷

- `docker run -it -v /宿主机目录:/容器内目录 ubuntu /bin/bash`，如
...
- 查看是否挂载成功，使用命令`docker inspect 容器id`

    ```json
    "Mount":[
        {
            "Type":"bind",	#表示绑定类型
            "Source":"/tmp/myHostData",	#表示宿主机内路径
            "Destination":"/tmp/myDockerData",	#表示Docker容器内路径
            "Mode":"",
            "RW":true,
...
- 挂载成功后，Docker内该路径下数据发生变化时，指定的宿主机路径中也会同步变化。反过来宿主机内的变化也会影响Docker容器

- 即使docker容器停止后，宿主机在进行变化，docker容器重启后仍能同步数据

###### 5、读写规则映射添加说明

- `docker run -it --privilege=true -v /宿主机目录:/容器内目录:ro ubuntu`，容器实例内部被限制，只能读取不能写，默认是rw，可以读写

- 只限制容器，不会限制宿主机，添加为ro时，容器自己只能读取不能写，不可以同步到宿主机。但此时如果宿主机写入内容，可以同步给容器内，容器可以读取到

###### 6、数据卷的继承和共享

- 容器2继承容器1的卷规则`docker run -it --privilege=true --volumes-from 父容器 --name=自定义子容器名`
- 子容器与父容器的映射规则相同，且在父容器关闭的情况下，仍可以与宿主机进行数据同步
