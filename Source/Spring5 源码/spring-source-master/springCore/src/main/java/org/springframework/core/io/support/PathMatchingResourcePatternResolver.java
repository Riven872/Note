...
public class PathMatchingResourcePatternResolver implements ResourcePatternResolver {
	...
	public Resource[] getResources(String locationPattern) throws IOException {
		...
		/*xxx: pathMatch分为两类： classpath*  以及 classpath  */
		if (locationPattern.startsWith(CLASSPATH_ALL_URL_PREFIX)) {
			// a class path resource (multiple resources for same name possible)
			/*xxx: 资源路径本身是否是通配，默认在通过 component扫描时，总是走通配的方法*/
	 ...
	 */
	/*xxx: 从给定的资源路径中，返回所有的资源句柄*/
	protected Resource[] findPathMatchingResources(String locationPattern) throws IOException {
		/*xxx: 获取根路径，实际上就是 **\/*.class之前的路径*/
		...
		for (Resource rootDirResource : rootDirResources) {
			...
			/*xxx: 获取正在遍历的根节点资源的统一资源描述符*/
			URL rootDirUrl = rootDirResource.getURL();
			/*xxx: 根节点资源可以是 bundle协议*/
	...
	protected String determineRootDir(String location) {
		...
		while (rootDirEnd > prefixEnd && getPathMatcher().isPattern(location.substring(prefixEnd, rootDirEnd))) {
			rootDirEnd = location.lastIndexOf('/', rootDirEnd - 2) + 1; /*xxx: 由于 最有有一个 **\*.class,因此，要跳过最后一个 /字符 */
		}
		if (rootDirEnd == 0) {
	 ...
	 * @see org.springframework.util.PathMatcher
	 */
	/*xxx: 从指定路径中，查找与指定正则相匹配的file资源,也是默认的资源策略*/
